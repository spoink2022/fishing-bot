...
const { PROD, clientId, discordToken, guildIds } = require('../private/config.json');
...
const commands = [
	...
	.addSubcommandGroup(subcommandgroup => subcommandgroup.setName('extension').setDescription('Expands the applicability of your swivel.')
		.addSubcommand(subcommand => subcommand.setName('whale').setDescription('Allows your swivel to work on whales.'))
		.addSubcommand(subcommand => subcommand.setName('prestige').setDescription('Allows your swivel to work on ALL prestige fish.'))
	),
...
if (!PROD) {
	...
	for (let guildId of guildIds) {
		rest.put(Routes.applicationGuildCommands(clientId, guildId), { body: commands })
			.then(() => console.log(`Successfully registered application commands for server with ID ${guildId}`))
			.catch(console.error);
	}
}