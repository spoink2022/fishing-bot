...
const { capitalizeWords } = require('./str_functions');
...
module.exports.createCanvasForAquarium = async function(locationID, aquariumFilter, fishContents) {
    ...
    for(const fish of Object.values(fishContents)) {
        ...
        let imgWidth = (S/10)*fish.screenLen;
        let imgHeight = imgWidth*(fishImg.height/fishImg.width);
        const x = ((W-imgWidth/2-W*0.05)-(imgWidth/2+W*0.05)) * Math.random() + (imgWidth/2+W*0.05);
        const y = ((H-imgHeight/2-H*0.1)-(imgHeight/2+H*0.1)) * Math.random() + (imgHeight/2+H*0.1);
...
module.exports.createCanvasForEquipment = async function(rodID, lineID, hookID, gloveID, equippedRing, premiumRing, swivelID, bannerID) {
    let canvas = createCanvas(W/2, H/2);
    let ctx = canvas.getContext('2d');

    // draw cosmetic banner
    if (bannerID || bannerID === 0) {
        let equipmentBannerImg = IMAGES.cosmetics.equipment_banner[bannerID.toString()];
        ctx.drawImage(equipmentBannerImg, 0, 0, W/2, H/2);
    }
    // draw defaults (rod, line, hook, glove, swivel, ring)
    ...
    if (gloveID !== 0) {
        ...
        ctx.drawImage(gloveImg, W/2-S/3.85, S/32, S/4, S/4);
    }

    if (equippedRing) {
        const ringImg = IMAGES.rings[equippedRing];
        let cropY = premiumRing ? 0 : 64;
        ctx.drawImage(ringImg,
            0, cropY, // start at (sx, sy) of the original image
            256, 320-cropY, // get (sw, sh) of pixels from the start point of the original image
            W/2-S/2+S/12, S/36+S/32 + (premiumRing ? 0 : S/24), // place at (dx, dy) on the canvas while accounting for padding
            S/6, S/6 * (premiumRing ? 1.25 : 1) // stretch to (dw, dh) on the canvas
        );
    }
    if (swivelID !== 0) {
...
module.exports.createSkinShowcaseCanvas = async function(img, W, H) {
    let canvas = createCanvas(W, H);
    ctx = canvas.getContext('2d');
...
module.exports.createSkinHelpCanvas = async function() {
    let canvas = createCanvas(540, 135);
    ctx = canvas.getContext('2d');

    ctx.drawImage(IMAGES.cosmetics.equipment_banner['0'], 0, 0, 180, 135);
    ctx.drawImage(IMAGES.cosmetics.equipment_banner['1'], 180, 0, 180, 135);
    ctx.drawImage(IMAGES.cosmetics.equipment_banner['2'], 360, 0, 180, 135);
