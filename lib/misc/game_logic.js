...
module.exports.getAquariumEarnings = async function(locationID, user) {
    ...
    let aquariumCapacity = Math.floor(AquariumInfo.max * (clan && this.getStarCount(clan.fish_caught) >= 5 ? 1.1 : 1));

    return Math.min(Math.round(earnings*10)/10, aquariumCapacity);
}
