...
module.exports.getAquariumFishTiers = async function(locationID, user) {
    ...
    for(let entry of LocationInfo.fish) {
        fishColumns.push(FishNames[entry.id].replace(' ', '_'));
    }
    let multiplierValues = await db.aquarium.getMultiplierValues(user.userid, fishColumns);
    let tierValues = {}, hourlyRate = 0;
    for(const[key, val] of Object.entries(multiplierValues)) {
        tierValues[key] = !val ? null : getTier(val);
    }
    return tierValues;
}

module.exports.getAquariumEarnRate = async function(locationID, user) {
    let fishTiers = await this.getAquariumFishTiers(locationID, user);
    let earnRate = 0;
    for(const val of Object.values(fishTiers)) {
        earnRate += val === null ? 0 : AquariumLevels[val];
    }
    return earnRate;
}
