...
module.exports.createCanvasForFishingPre = async function(user, possibleFish, sizeAdjustment=1, rodSize, bait) {
    ...
    if (user.swivel !== 0) {
        ...
        ctx.drawImage(swivelImg, rodOffset*W-S/16/sizeAdjustment, H*0.2, S/8/sizeAdjustment, S/6/sizeAdjustment);
    }
    // Draw bait (if any)
    if (bait) {
...
module.exports.createFishShowcaseCanvas = async function(fishImg, screenLen, zoom) {
    ...
    scale *= Math.min(1, 0.25 + (screenLen/20)*3/4 * Math.max(1, zoom/1.5));
