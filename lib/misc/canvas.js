...
module.exports.createCanvasForFishingPre = async function(user, possibleFish, sizeAdjustment=1) {
    ...
    ctx.drawImage(rodImg, W*rodOffset, H*0.05, W/4/sizeAdjustment, H/4/sizeAdjustment);
    // Draw line
...
module.exports.createCanvasForFishingPost = async function(locationID, oldCanvas, fish, lineSnapped, zoom) {
    ...
    scale = scale * tierMultipliers[fish.tier] * Math.min(1, fish.originalScreenLen/10 * zoom);
...
module.exports.createFishShowcaseCanvas = async function(fishImg, screenLen, zoom) {
    ...
    if(fishImg.width/fishImg.height > W/(H/2)) { // fish is wider than area (scale to width)
        var scale = W/fishImg.width;
    } else { // fish is taller than area (scale to height)
        var scale = H*0.5/fishImg.height;
    }
    scale *= Math.min(1, 0.25 + (screenLen/20)*3/4 * zoom);
    let x = (W-fishImg.width*scale)/2, y = (H/2-fishImg.height*scale)/2;
