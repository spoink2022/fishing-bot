...
const S = 150; // defaults to a chunk (4x3 chunks in a standard canvas area)
...
module.exports.createEquipmentCanvas = async function(user, ringType, bannerID) {
    ...
    ctx.moveTo(1*S, 0.3*S);
    ctx.lineTo(1*S, 2.4*S);
    ctx.stroke();
    // Rod, Hook
    const rodImg = IMAGES.rods[user.rod];
    ctx.drawImage(rodImg, 1*S, 0.3*S, 2.24*S, 1.68*S);
    const hookImg = IMAGES.hooks[user.hook];
    ctx.drawImage(hookImg, 0.92*S, 2.4*S, 0.16*S, 0.24*S);
    // Swivel, Gloves, Ring
    if (user.swivel !== 0) {
