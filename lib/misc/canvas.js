...
module.exports.createEquipmentCanvas = async function(user, ringType, bannerID) {
    ...
    if (user.prestige_swivel) {
        const prestigeSwivelImg = IMAGES.prestige_swivel;
        ctx.drawImage(prestigeSwivelImg, 0.95*S, 1.4*S, 0.1*S, 0.15*S);
    }
    if (user.gloves !== 0) {
...
module.exports.createFishingCanvasPre = async function(user, locationId, baitName, weatherId, generatedFish) {
    ...
    // Draw whale swivel extension
    if (user.whale_swivel) {
        const whaleSwivelImg = IMAGES.whale_swivel;
        ctx.drawImage(whaleSwivelImg, xHook - 0.025*S*scale, 0.5*S, 0.05*S*scale, 0.075*S*scale);
    }
    // Draw prestige swivel extension
    if (user.prestige_swivel) {
        const prestigeSwivelImg = IMAGES.prestige_swivel;
        ctx.drawImage(prestigeSwivelImg, xHook - 0.025*S*scale, 0.4*S, 0.05*S*scale, 0.075*S*scale);
    }
    // Draw bait
    if (baitName) {
...
module.exports.createItemUpgradeCanvas = async function(itemType, itemId) { // itemId of the old item
    ...
    else if (['line', 'hook', 'swivel', 'whale', 'prestige'].includes(itemType)) { canvas = createCanvas(4*S, 3*S); }
    ...
    switch (itemType) {
        ...
        case 'whale':
            ...
            break;
        case 'prestige':
            ctx.drawImage(IMAGES.prestige_swivel, 1*S, 0, 2*S, 3*S);
            break;
        case 'aquarium':
