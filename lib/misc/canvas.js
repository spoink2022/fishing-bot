...
module.exports.createFishingCanvasPre = async function(user, locationId, baitName, weatherId, generatedFish) {
    ...
    ctx.drawImage(rodImg, xHook, 0.2*S, 0.8*S, 0.6*S);
    // Draw line
    const LineData = api.equipment.getLineData(user.line);
    ctx.strokeStyle = LineData.rgb;
    ctx.lineWidth = S*0.005*LineData.px;
    ...
    ctx.drawImage(hookImg, xHook - 0.05*S, yHook, 0.1*S, 0.15*S);
    // Draw swivel
    if (user.swivel) {
        const swivelImg = IMAGES.swivels[user.swivel];
        ctx.drawImage(swivelImg, xHook - 0.05*S, 0.6*S, 0.1*S, 0.15*S);
    }
    // Draw bait
    if (baitName) {
        const baitImg = IMAGES.bait[baitName];
        ctx.drawImage(baitImg, xHook - 0.05*S, yHook, 0.1*S, 0.15*S);
    }
    
    return canvas;
}

module.exports.createFishingCanvasPost = async function(oldCanvas, fish, lineSnapped) {
    ...
    ctx.stroke();
    // tape strips
    ctx.drawImage(IMAGES.tapeStrip, 1.84*S, 1.2*S, 0.5*S, 0.5*S);
    ctx.drawImage(IMAGES.tapeStrip, 0.08*S, 2.46*S, 0.5*S, 0.5*S);
    ctx.save();
    ctx.translate(0.58*S, 1.2*S);
    ctx.scale(-1, 1);
    ctx.drawImage(IMAGES.tapeStrip, 0, 0, 0.5*S, 0.5*S);
    ctx.restore();
    ctx.save();
...
module.exports.createInfoCanvas = async function(itemType, itemId) {
    ...
    } else if (['bait', 'aquarium', 'rod'].includes(itemType)) {
    ...
    switch (itemType) {
        case 'fish':
            ...
            scale *= 0.7 * Math.min(1, FishData.screenLen/10);
        ...
        case 'engine':
            ...
            break;
        case 'container':
            ctx.drawImage(IMAGES.container[itemId], 0.4*S, 0.2*S, 3.2*S, 1.6*S);
            break;
        case 'propeller':
...
module.exports.createSkinCanvas = async function(category, id) {
    ...
    ctx.drawImage(IMAGES.cosmetics[category][id], 0, 0, 4*S, 3*S);
