...
module.exports.parseBaitList = function(baitList) {
    let baitObj = {};
    let count = 0;
    for(const key of Object.keys(api.gamedata.getAllBaitData())) {
        baitObj[key] = baitList[count];
        count++;
    }
    return baitObj;
}

module.exports.parseBaitObj = function(baitObj) {
    let baitList = [];
    for(const val of Object.values(baitObj)) {
        baitList.push(val);
    }
    return baitList;
}