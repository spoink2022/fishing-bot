...
async function preloadCosmeticImages() {
    IMAGES.cosmetics = {};
    const cosmeticNames = api.cosmetics.getAllCosmeticFiles();
    let tasks = [];
    for (let i=0; i<cosmeticNames.length; i++) {
        tasks.push(loadCosmeticImage(cosmeticNames[i]));
    }
    Promise.all(tasks).then(() => console.log('preloaded cosmetic images!'));
}
async function loadCosmeticImage(name) {
    IMAGES.cosmetics[name] = await loadImage(api.images.fetchCosmeticUrl(name));
}
