...
async function preloadImages() {
    ...
    const miscImages = [
        'tape', 'fish_escaped', 'caught_fish_bg', 'caught_fish_bg_red', 'caught_fish_bg_green', 'caught_fish_bg_purple',
        'wanted', 'sun', 'cloud', 'rain', 'boat_docked', 'timeout', 'whale_swivel', 'prestige_swivel'
    ];
    let tasks = [];
...
async function preloadBaitImages() {
    ...
    for (let i=0; i<chumBaitNames.length; i++) {
        tasks.push(loadChumBaitImage(chumBaitNames[i]));
    }
...
async function loadChumBaitImage(baitName) {
    IMAGES.bait[baitName] = await loadImage(api.images.fetchBaitImgUrl('chum/' + baitName));
    return;
}
