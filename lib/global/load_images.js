...
async function preloadFishImages() {
    IMAGES.fish = {};
    const fishImageCounts = api.fishing.getFishImageCounts();
    for(let i=0; i<fishImageCounts.length; i++) {
        IMAGES.fish[i.toString()] = {};
        for(let j=0; j<fishImageCounts[i]; j++) {
            IMAGES.fish[i.toString()][j.toString()] = await loadImage(api.images.fetchFishImgUrl(i, j));
        }
    }
