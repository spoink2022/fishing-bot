...
module.exports.sendOptInCommnad = async function(interaction, user) {
    // OPT IN
    if (user.opted_in) { return interaction.reply('You are already opted in!'); }
    await db.users.setColumns(user.userid, { opted_in: true });
    let embed = {
        color: logic.color.STATIC.success,
        title: 'You are now opted in!',
        description: 'Your stats, aquariums, and equipment can now be viewed by members of servers you are in.'
    };
    interaction.reply({ embeds: [embed] });
}
...
module.exports.sendOptOutCommnad = async function(interaction, user) {
    // OPT OUT
    if (!user.opted_in) { return interaction.reply('You are already opted out!'); }
