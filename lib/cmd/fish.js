...
async function createPostFishingEmbed(user, author, clan, fish, oldCanvas, gloveBonus, currentEvent, baitLabel) {
    ...
    if (!lineSnapped && user.quest_type !== -1 && user.quest_progress !== user.quest_requirement) {
        if (user.quest_type === 0 && fish.r >= [0.3, 0.6, 0.75, 0.9][user.quest_data]) { // catch tier
            ...
            newQuestProgress = Math.min(user.quest_progress + fish.weight, user.quest_requirement).toFixed(3);
    ...
    if (!lineSnapepd && clan && user.level >= 10) {
        clanString = '\n+1 clan catch! :fish:';
    ...
    return [{
        color: logic.color.STATIC.location[user.location - 1],
        title: `${lineSnapped ? 'Oh no! Your line snapped on' : 'Caught'} a${['a', 'e', 'i', 'o', 'u'].includes(FishData.name[0]) ? 'n' : ''} ${FishData.name.replace(/_/g, ' ')}!`,
        author: {
