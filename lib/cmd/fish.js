...
${gloveBonus ? `:gloves: Gloves Activated! (+${GloveData.bonus}kg Max)` : ''}`,
    ...
    return sendReply(interaction, { embeds: [embed], files: [attachment], components: [row] }).then(async (sentMessage) => {
        let [embedArr, attachment2] = await createPostFishingEmbed(user, interaction.user, clan, generatedFish[0], canvas, gloveBonus, currentEvent, baitLabel, interaction.guild ? interaction.guild.id : false);
