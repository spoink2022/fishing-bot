...
module.exports.sendFishCommand = async function(interaction, user, baitName) {
    ...
    if (baitName) {
        ...
        if (!BaitData) { return interaction.reply(`**${baitLabel}** is not a valid bait!`); }
...
${gloveBonus ? `:gloves: Gloves Activated! (+${GloveData.bonus}kg Max)` : ''}`,
    ...
    return interaction.reply({ embeds: [embed], files: [attachment], components: [row] }).then(async (sentMessage) => {
        let [embed2, attachment2] = await createPostFishingEmbed(user, interaction.user, clan, generatedFish[0], canvas, gloveBonus, currentEvent, baitLabel, interaction.guild ? interaction.guild.id : false);
        ...
        collector.on('collect', async i => {
            collector.stop();
            if (interaction.constructor.name === 'Message') { return sentMessage.edit({ embeds: [embed2], files: [attachment2], components: [] }); }
...
async function createPostFishingEmbed(user, author, clan, fish, oldCanvas, gloveBonus, currentEvent, baitLabel, serverId) {
    // Create Variables
    ...
    if (baitLabel) {
        const BaitData = api.bait.getBaitData(baitLabel.replace(/ /g, '_').toLowerCase());
