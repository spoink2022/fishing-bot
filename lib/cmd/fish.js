...
module.exports.sendFishCommand = async function(interaction, user, baitName) {
    ...
    if (baitName) {
        ...
        if (!BaitData) { BaitData = api.bait.getChumBaitData(baitName); }
...
async function createAttachment2(user, clan, fish, gloveBonus, currentEvent, baitLabel, oldCanvas) {
    ...
    fish.weight *= 10 ** (3 * user.prestige);
    ...
    if (FishData.family === 'shark' || (FishData.family === 'whale' && user.whale_swivel) || (user.prestige > 0 && user.prestige_swivel)) {
        const SwivelData = api.equipment.getSwivelData(user.swivel);
        if (SwivelData) { maxWeight += SwivelData.bonus; }
    }
    let baitName, BaitData;
    if (baitLabel) {
        baitName = baitLabel.replace(/ /g, '_').toLowerCase();
        BaitData = api.bait.getBaitData(baitName) || api.bait.getChumBaitData(baitName);
        if (BaitData.bonus) { maxWeight *= (1 + BaitData.bonus/100); }
    }
    const lineSnapped = fish.weight > maxWeight;

    // Render Canvas
    const canvasBuffer = await createFishingCanvasPost(oldCanvas, fish, lineSnapped, user.prestige);
...
async function createPostFishingEmbed(user, author, clan, fish, gloveBonus, currentEvent, baitLabel, serverId) {
    ...
    const prestigeMultiplier = user.prestige == 1 ? 0.35 : 1;
    let coins = lineSnapped ? 0 : Math.ceil(fish.weight * FishData.value * HookData.multiplier * prestigeMultiplier);
    
    const LineData = api.equipment.getLineData(user.line);
    let exp = lineSnapped ? 0 : Math.ceil(Math.sqrt(coins * 10)) + LineData.bonus;
    if (BaitData && BaitData.coin_exp) { exp = Math.ceil(exp * (1 + BaitData.coin_exp/100)); }
    const bonusExp = Math.ceil(exp * logic.clan.getExpBonus(clan)/100);

    if (BaitData && BaitData.coin_exp) { coins = Math.ceil(coins * (1 + BaitData.coin_exp/100)); }
    ...
    while (newExp >= expRequired) {
        ...
        if (newLevel % 10 === 0 && newLevel < 140) { levelUpString += '\nUnlocked a new location! :map:' }
        if (newLevel === 140) { levelUpString += '\nUnlocked Prestige! (See `/help prestige`) :dna:' }
    }
    // handle quest
    let questString = ''
    ...
    if (!lineSnapped && user.ring) {
        ...
        if (RingData.dropMultiplier) {
            cardDropChance *= RingData.dropMultiplier;
        }
