...
const { sendBuyEquipmentCommand, sendBuyRingCommand } = require('./buy.js');
...
const { sendShopEquipmentCommand } = require('./shop.js');
...
module.exports.handleCommandInteraction = async function(interaction) {
    ...
    switch (interaction.commandName) {
        ...
        case 'buy':
            if (['aquarium', 'rod', 'line', 'hook', 'gloves', 'swivel'].includes(interaction.options.getSubcommand())) {
                sendBuyEquipmentCommand(interaction, user);
            } else {
                switch (interaction.options.getSubcommandGroup()) {
                    case 'ring':
                        sendBuyRingCommand(interaction, user);
                        break;
                }
            }
            break;
