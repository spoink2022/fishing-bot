...
const cmdObjects = [Profile, Game, Info];
...
module.exports = async function(msg, runHelp=false) {
    ...
    if(validCommands.includes(cmd) || cmd === 'init') {
        ...
        const checkData = await runCheck(msg.author.id); // returns [can_run_function, remaining_cooldown]
        if(!checkData[0]) {
            if(msg.channel.type === 'dm') {
                msg.channel.send(`You're messaging too fast! Please wait ${Math.round(checkData[1]/100, 1)/10}s`);
            } else { msg.reply(`You're messaging too fast! Please wait ${Math.round(checkData[1]/100, 1)/10}s`);  }
            return;
        }
