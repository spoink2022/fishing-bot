...
module.exports.sendRedeemCommand = async function(interaction, user) {
    ...
    if (server.custom_fish_privilege) { return sendReply(interaction, 'This server already has a Permanent Server Boost!'); }
...
module.exports.sendSetCustomFishCommand = async function(interaction, user, command) {
    ...
    const server = await db.servers.fetchServer(interaction.guild.id);
    if (!server.custom_fish_privilege) { return sendReply(interaction, 'This server is does not have this perk! Please visit https://bigtuna.xyz/shop to boost your server!'); }
