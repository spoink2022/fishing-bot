...
module.exports.sendBuyEquipmentCommand = async function(interaction, user, category) {
    ...
    if (!data) { // not valid entry
        ...
        return interaction.reply(`You must be at least **Lvl. ${data.level}** to purchase **${logic.text.capitalizeWords(`${data.name} ${category}`)}**!`);
    } else if (user.coins < data.price) { // too expensive
        return interaction.reply(`You must have at least **${data.price} coins** to purchase **${logic.text.capitalizeWords(`${data.name} ${category}`)}**!`);
    }
