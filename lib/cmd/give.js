...
module.exports.sendGiveSkinCommand = async function(interaction, user) {
    ...
    const skinId = interaction.options.getString('id').toUpperCase();
...
module.exports.sendGiveSupporterCommand = async function(interaction, user) {
    // GIVE: SUPPORTER
    // Step 1 - Validate User Argument
    const mentionedUser = interaction.options.getUser('user');
    if (mentionedUser.bot) {
