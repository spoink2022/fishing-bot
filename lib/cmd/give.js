...
module.exports.sendGiveCardCommand = async function(interaction, user, cardId, mentionedUser) {
    ...
    if (!cardId) { return interaction.reply('You must specify the ID of the card to give!'); } // text-based command calls
...
module.exports.sendGiveRingCommand = async function(interaction, user, ringId, mentionedUser) {
    ...
    if (!ringId)  { return interaction.reply('You must specify the ID of the ring to give!'); } // text-based command calls
...
module.exports.sendGiveSkinCommand = async function(interaction, user, skinId, mentionedUser) {
    const CATEGORY_MAP = { 'A': 0 };
    const CATEGORY_ARR = ['equipment_banner'];
    // GIVE: SKIN
    // Step 1 - Validate User Argument
    if (mentionedUser.bot) {
        return interaction.reply(`**${mentionedUser.username}** is a bot and cannot have a Big Tuna account!`);
    } else if (mentionedUser.id === interaction.user.id) {
        return interaction.reply('You cannot give a skin to yourself!');
    }
    const otherUser = await db.users.fetchUser(mentionedUser.id);
    if (!otherUser) {
        return interaction.reply(`**${mentionedUser.username}** has not used Big Tuna before!`);
    ...
    if (!skinId) { return interaction.reply('You must specify the ID of the skin to give!'); } // text-based command calls
    skinId = skinId.toUpperCase();
    const number = skinId.slice(0, -1);
    const categoryId = CATEGORY_MAP[skinId.slice(-1)];
    if (categoryId === undefined || parseInt(number) != number) { return interaction.reply(`**${skinId}** is not a valid skin id! Use\`/skins\` to view your skins.`); }
    const skin = await db.skins.getSkinByRelativeId(user.userid, categoryId, parseInt(number));
    if (!skin) { return interaction.reply(`You have no skin with id **${skinId}**! Use\`/skins\` to view your skins.`); }
    ...
    if (userSkins.length >= 10) { return interaction.reply(`**${mentionedUser.username}** already has **10 ${CategoryData.name.toLowerCase()}**!`); }
...
module.exports.sendGiveSupporterCommand = async function(interaction, user, mentionedUser) {
    ...
    if (mentionedUser.bot) {
        ...
        return interaction.reply('You cannot give premium to yourself!');
    }
    const otherUser = await db.users.fetchUser(mentionedUser.id);
    if (!otherUser) {
        return interaction.reply(`**${mentionedUser.username}** has not used Big Tuna before!`);
    ...
    if (user.big_supporter <= 1) { return interaction.reply('You must have at least 1 extra Big Supporter to gift them!'); }
