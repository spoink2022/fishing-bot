...
async function sendSetPrefix(msg, args) {
    ...
    if (!msg.member.hasPermission('MANAGE_GUILD')) {
        attemptReply(msg, 'You must have `Manage Server` permissions in order to use this command!');
        return;
    }
    if (!ACCEPTED_PREFIXES.includes(args[0])) {
        attemptReply(msg, `You must specify a prefix from the set: \`${ACCEPTED_PREFIXES.join(' ')}\``);
        return;
    }
...
async function sendSetCustomFish(msg, args) {
    ...
    if (!customFishCommand) {
        if (server.custom_fish) {
            ...
            attemptReply(msg, 'You must provide a string to map the command to!');
        }
        return;
    } else if (customFishCommand.length > 32) {
        attemptReply(msg, `\`${customFishCommand}\` exceeds 32 characters!`); return;
    }

    db.servers.setCustomFishCommand(msg.guild.id, customFishCommand);
    updateCustomFishLocals(msg.guild.id, customFishCommand);

    msg.channel.send(`\`${customFishCommand}\` will now trigger fishing!`);
}