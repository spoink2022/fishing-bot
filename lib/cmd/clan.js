    ...
    msg.channel.send(embed).then(async (sentEmbed) => {
        ...
        collector.on('collect', async (reactionData) => {
            ...
            let options = {
                author: [clan.name, null],
                color: '#2acaea',
                title: `Purchased ${perkInfo[1].shopName} ${isUpgrade ? 'Upgrade' : 'x1'}!`
            };
            let embed = await createEmbed(options);
            msg.channel.send(embed);
        });
...
${perks.campaign_mba ? `\n**+${perks.campaign_mba}** clan points from campaigns :receipt:` : ''}
${perks.aquarium_capacity ? `\n**+${perks.aquarium_capacity}%** aquarium coin capacity` : ''}
${perks.max_weight ? `\n**+${perks.max_weight}%** max weight` : ''}`;
