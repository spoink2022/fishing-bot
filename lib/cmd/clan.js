...
const { createClanBoatCanvas } = require('../misc/canvas.js');
...
module.exports.sendClanCommandMessage = async function(msg, user) {
    ...
    switch (cmd) {
        case 'profile':
            ...
            sendProfile(msg, user, clanMember, mentionedUser);
            break;
        case 'members':
...
async function sendCreate(interaction, user, clanMember, clanName) {
    ...
    if (!clanName) { return interaction.reply('You must provide a name for your clan!'); } // text-based command calls
...
async function sendJoin(interaction, user, clanMember, clanId, password) {
    ...
    if (!clanId) { return interaction.reply('You must provide a clan id!'); } // text-based command calls
    if (!parseInt(clanId)) { return interaction.reply('A clan ID must be a **number**!'); } // text-based command calls
    const clan = await db.clan.fetchClan(clanId);
    if (!clan) { return interaction.reply(`There is no clan with id **${clanId}**!`); }
    if (clan.password && !password) { return interaction.reply('This clan requires a password to join!'); }
...
async function sendLeave(interaction, user, clanMember) {
    ...
    if (clanMember.role === 2 && members.length !== 1) { return interaction.reply('Promote a player to Clan Leader before leaving your clan!'); }    
    // Step 2 - Send Confirmation Embed
...
async function sendRename(interaction, user, clanMember, clanName) {
    ...
    if (!clanName) { return interaction.reply('You must provide the new name of your clan!'); } // text-based command calls
    if (!isValidName(clanName)) {
        let embed = {
            color: logic.color.STATIC.failure,
            title: `"${clanName}" does not meet clan name requirements!`,
...
async function sendPromote(interaction, user, clanMember, memberId) {
    ...
    if (!memberId) { return interaction.reply('You must specify the id of the member to promote!'); } // text-based command calls
    if (!parseInt(memberId)) { return interaction.reply('A member ID must be a **number**!'); } // text-based command calls
    const members = await db.clan.fetchMembers(clanMember.clan);
...
async function sendDemote(interaction, user, clanMember, memberId) {
    ...
    if (!memberId) { return interaction.reply('You must specify the id of the member to demote!'); } // text-based command calls
    if (!parseInt(memberId)) { return interaction.reply('A member ID must be a **number**!'); } // text-based command calls
    const members = await db.clan.fetchMembers(clanMember.clan);
    if (memberId <= 0 || memberId > members.length) {
        return interaction.reply(`**${memberId}** is not a valid member ID! See \`/clan members\` for a list of member IDs.`);
...
async function sendKick(interaction, user, clanMember, memberId) {
    ...
    if (!memberId) { return interaction.reply('You must specify the id of the member to kick!'); } // text-based command calls
    if (!parseInt(memberId)) { return interaction.reply('A member ID must be a **number**!'); } // text-based command calls
    const members = await db.clan.fetchMembers(clanMember.clan);
    if (memberId <= 0 || memberId > members.length) {
        return interaction.reply(`**${memberId}** is not a valid member ID! See \`/clan members\` for a list of member IDs.`);
...
async function sendBuy(interaction, user, clanMember, category) {
    ...
    if (clanMember.role === 0) { return interaction.reply('You must be a **Trusted Member** or the **Clan Leader** to buy clan perks!'); }
    const BUY_MAP = { 'cooldown': 'fish_cd', 'coin': 'coin_bonus', 'exp': 'exp_bonus', 'vote': 'vote_bonus', 'campaign': 'campaign_mba', 'quest': 'quest_mba' };
    const clan = await db.clan.fetchClanByUserid(user.userid);
    if (!category) { return interaction.reply('You must specify the perk you wish to purchase!'); } // text-based command calls
    const perkName = BUY_MAP[category];
    if (!perkName) { return interaction.reply(`**${category}** is not a valid category!`); } // text-based command calls
    ...
    if (!nextLevel)  { return interaction.reply('Your clan has already maxed out that perk!'); }
    if (nextLevel.price > clan.clan_points) { return interaction.reply('Not enough clan points!'); }
...
async function sendBoat(interaction, user, clanMember) {
    ...
    const canvasBuffer = await createClanBoatCanvas(clan, status);
    const attachment = new MessageAttachment(canvasBuffer, 'clan_boat.png');

    // Step X-1 - Embed Fields
    let embedFields = [];
    embedFields.push({
...
:alarm_clock: Cooldown: ${logic.text.millisToDaysAndHours(PropellerData.cooldown)}`,
        inline: true
    });
    embedFields.push({ name: '\u200b', value: '\u200b', inline: true });
    // Step X - Send Embed
    let embed = {
        color: logic.color.STATIC.clan,
        title: `Clan Boat (${['Idle', 'Phase 1/2: Fishing', 'Phase 2/2: Unloading'][status]})`,
        description: `:fuelpump: Fuel Available: ${clan.fuel} units
:map: Destination: ${status === 1 ? `${ClanLocationData.name} (${clan.boat_location})` : 'None'}`,
        fields: embedFields,
        image: {
