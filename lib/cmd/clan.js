...
const { MessageAttachment, MessageActionRow, MessageButton, Message } = require('discord.js');

const api = require('../../api');
const db = require('../../db');
const logic = require('../logic');

const { calculateFishWeight, getMaxWeight } = require('../misc/calculations.js');
...
async function handleBoatClaimButton(clan, clanMember, user, interaction) {
    ...
    let rewardMessages = rewards.map(obj => {
        if (obj.rewardType === 'fish') {
            ...
            let recordStr = '';
            if (obj.r > FishValues[FishData.name]) {
                recordStr = ':truck:';
            ...
            if (ring) {
                ...
                if (Math.random() * 100 < cardDropChance) {
                    if (cardCount >= 10) { // too many cards
            ...
            if (!lineSnapped && user.quest_type !== -1 && user.quest_progress !== user.quest_requirement) {
                if (user.quest_type === 0 && fish.r >= [0.3, 0.6, 0.75, 0.9][user.quest_data]) {
