...
module.exports.sendClanCommand = async function(interaction, user) {
    ...
    if (interaction.options.getSubcommandGroup(false) === 'buy') { return sendBuy(interaction, user, clanMember); }
...
async function sendBuy(interaction, user, clanMember) {
    if (!clanMember) { return interaction.reply('You are not currently in a clan! For more information, use `/help clan`.'); }
    // CLAN: SHOP
    ...
    const perkName = BUY_MAP[interaction.options.getSubcommand()];
