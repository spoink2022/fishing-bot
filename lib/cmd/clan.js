...
module.exports.sendClanCommandInteraction = async function(interaction, user) {
    ...
    if (clanMember && clanMember.tag !== interaction.user.username) {
        clanMember.tag = interaction.user.username;
...
async function sendJoin(interaction, user, clanMember, clanId, password) {
    ...
    await db.clan.createClanMember(user.userid, `${interaction.user.username}`, clanId);
