...
module.exports.run = async function(msg, cmd, args, user) {
    ...
    else if (cmd === 'createclan') { sendCreateClan(msg, user); }
...
async function sendCreateClan(msg, user) {
...
async function sendRename(msg, args) {
    ...
    // renames left?
    let clan = await db.clan.fetchClan(clanMember.clan);
    if (clan.rename === 0) { attemptReply(msg, 'Your have no clan renames left!\nPlease wait until you reach the next clan rank!'); return; }
    ...
    // case: >32 characters
    if (chosenName.length > 32) { attemptReply(msg, 'Your clan name may not have more than 32 characters!'); }
        ...
        collector.on('collect', async (reactionData) => {
            if (reactionData.emoji.name === '✅') {
                ...
                var options = {
                    title: 'Success!',
                    color: api.visuals.getColor('classic', 'success'),
                    description: `Success! Your clan has been renamed to **${chosenName}**!`
                }
        ...
        msg.channel.send(embed).then(async(sentEmbed) => {
            ...
            collector.on('collect', async (reactionData) => {
                if (reactionData.emoji.name === '✅') {
                    ...
                    proxyMentionedClanMember = await db.clan.fetchMember(mentionedUser.user.id);
