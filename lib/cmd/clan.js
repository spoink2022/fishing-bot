...
const { getStarCount, getClanPerks } = require('../logic/game.js');
...
function generateActivityMarker(days, epochDay, joinEpochDay, lastCampaignCatch) {
    if (epochDay === joinEpochDay) { return ':white_large_square:'; }
    if (epochDay === lastCampaignCatch) { return ':blue_square:'; }
    if (days < 7) { return ':green_square:'; }
    if (days < 14) { return ':yellow_square:'; }
    return ':red_square:';
}
...
async function sendClan(msg) {
    ...
    // post clan info
    let epochDay = datetime.getEpochDay();
    ...
    let options = {
        ...
        description: embedDescription + clanMemberLevels.map(levelStats => {
            ...
            const joinEpochDay = datetime.getEpochDayFromString(clanMember.date);
            let activityMarker = generateActivityMarker(days, epochDay, joinEpochDay, clanMember.last_campaign_catch);
            return `${activityMarker} ${ROLE_EMOJIS[clanMember.role]} ${bold}${clanMember.username} (Lvl. ${levelStats.level})${bold}`;
        }).join('\n'),
        footer: `Campaign cooldown resets in ${datetime.millisToTimeString((epochDay+1)*24*60*60*1000 - Date.now())}`
    };
...
async function sendClanShop(msg) {
    ...
    let options = {
        ...
        fields: embedFields,
        footer: clanMember.role === 0 ? 'You must be a Trusted Member or Clan Leader to purchase upgrades' : ''
    };
    let embed = await createEmbed(options);
...
async function sendClanPerks(msg) {
    ...
    let embedDescription = `${perks.fish_cd ? `\n**-${perks.fish_cd}%** fishing cooldown :alarm_clock:` : ''}\
...
${perks.max_weight ? `\n**+${perks.max_weight}%** max weight :bricks:` : ''}`;
...
Keep an eye out for updates, **more is coming soon!**'
    ...
    for (let entry of campaignInfo.requirements) {
        ...
        campaignProgress[fishName] = Math.min(campaignProgressIDs[entry[0]], entry[1]) || 0;
    ...
    **Rewards**`;
    ...
    msg.channel.send(embed).then(async (sentEmbed) => {
        ...
        collector.on('collect', async (reactionData) => {
            collector.stop();
            if (clanMember.role == 0) {
                attemptReply(msg, 'The Clan Leader :crown: or a Trusted Member :reminder_ribbon: must be the one to claim rewards and accept the next campaign stage!');
                return;
            }
