...
module.exports.run = async function(msg, cmd, args, user) {
    if (cmd === 'clan') { showClan(msg); } // .clan checks for username change
...
async function showClan(msg) {
    ...
    let options = {
        author: [`${capitalizeWords(clan.name)}`, null],
        title: 'Clan Overview',
        color: '#2acaea',
...
async function sendJoin(msg, args) {
    ...
    if (!args[0]) { attemptReply(msg, 'You must specify the name or the ID of the clan you wish to join!'); return; }
    if (isNaN(args[0])) { // clan by name
        var clan = await db.clan.fetchClanByName(args.join(' ')) || await db.clan.fetchClanByName(args.slice(0, args.length-1).join(' '));
        if (!clan) { attemptReply(msg, `There is no clan with that name!\n*Make sure your capitalization is correct!*`); return; }
    } else { // clan by id
        var clan = await db.clan.fetchClan(args[0]);
        if (!clan) { attemptReply(msg, `There is no clan with Clan ID \`${args[0]}\``); return; }
    }
    // password check
    if (clan.password) {
        if (clan.password !== args[args.length-1]) {
            attemptReply(msg, 'You must provide a correct password in addition to the clan name to join this clan!'); return;
        }
    }
    // member cap check
    let memberCount = (await db.clan.fetchMembers(clan.id)).length;
    if (memberCount >= 20) { attemptReply(msg, 'This clan already has 20 members!'); }
    // add to clan, send message
    db.clan.joinMember(msg.author.id, msg.author.username + '#' + msg.author.discriminator, clan.id);
    attemptReply(msg, `Congratulations! You joined the clan **${capitalizeWords(clan.name)}**!`);
}
...
async function sendLeave(msg) {
    ...
    if (clanMember.role !== 2) {
        ...
        attemptReply(msg, `Success! You left the clan **${capitalizeWords(clan.name)}**!`);
    } else {
...
async function sendPassword(msg) {
    ...
    // post information
    if (!clan.password) {
        msg.channel.send(`There is no join password for the clan **${capitalizeWords(clan.name)}**`);
    } else if (clanMember.role === 0) {
        attemptReply(msg, 'You must be a trusted member or a clan leader to view your clan join password!');
    } else {
        msg.channel.send(`The join password for the clan **${capitalizeWords(clan.name)}** is \`${clan.password}\``);
    }
}
...
async function sendNoPassword(msg) {
    ...
    let clan = await db.clan.fetchClan(clanMember.clan);
    if (!clan.password) { attemptReply(msg, `Your clan **${capitalizeWords(clan.name)}** already does not require a password to join!`); return; }
    // make the changes, post information
    db.clan.setPassword(clanMember.clan, null);
    attemptReply(msg, `Players can now join the clan **${capitalizeWords(clan.name)}** without a password!\nTo go back to requiring a password, use \`.newpassword\``);
}
