...
async function sendJoin(interaction, user) {
    if (user.clan !== 0) { return interaction.reply('You are already in a clan!'); }
    // CLAN: JOIN
...
async function sendLeave(interaction, user, clanMember) {
    if (user.clan === 0) { return interaction.reply('You are not in a clan!'); }
    // CLAN: LEAVE
    ...
    interaction.reply({ embeds: [embed], components: [row] }).then(async () => {
        ...
        collector.on('collect', async i => {
            ...
            let embed2;
            if (action === 'confirm') {
                embed2 = await handleLeaveConfirmButton(user, clan.name);
            } else {
...
async function handleLeaveConfirmButton(user, clanName) {
    ...
    await db.users.setColumns(user.userid, { clan: 0 });
    // Return Embed
    return {
        color: logic.color.STATIC.success,
        title: 'Left Clan!',
