...
module.exports.run = async function(msg, cmd, args, user) {
    ...
    else if (cmd === 'skin') { sendSkin(msg, user, args); }
}
...
async function sendSkin(msg, user, args) {
    if (!args[0]) {
    ...
    if (!category) {
        attemptReply(msg, 'That is not a valid skin ID! Check `.skins` to view your skins.');
        return;
    }
    ...
    let globalSupply = await db.cosmetics.getCosmeticCount(category, skin.cosmetic_id);
    ...
    msg.channel.send(embed).then(async (sentEmbed) => {
        ...
        collector.on('collect', async(reactionData) => {
            ...
            if (reactionData.emoji.name === '⤵️') { //unequip skin
                ...
                await db.cosmetics.equipCosmetic(skin.id, category);
                var options = {
...
async function sendAllSkins(msg, user) {
    ...
    for (let skin of skins) {
        ...
        embedFieldsObj[skin.category] += `\n\`${counts[skin.category]}${getCharFromNum(Data[skin.category].index)}\` - ${skinData.name} ${skin.equipped ? ':white_check_mark:' : ''}`;
    }
    let embedFields = [
        { name: `Equipment Banners (${counts.equipment_banner}/5)`, value: embedFieldsObj.equipment_banner }
    ];
    ...
    let options = {
        ...
        color: api.visuals.getNestedColor('cmd', 'stats', definingPurchase || 'default'),
        fields: embedFields,
        footer: 'For information about a specific skin, use .skin [ID]'
    };
    let embed = await createEmbed(options);
    msg.channel.send(embed);
}