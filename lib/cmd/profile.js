...
const { getStarCount, getTier, calculateAvgRingChance, calculateAvgRingMultiplier } = require('../misc/game_logic.js');
...
async function sendEquipment(msg, user) {
    ...
    let clanBonus = clan && getStarCount(clan.fish_caught) >= 6 ? 1.05 : 1;
    ...
    let canvasBuffer = await createCanvasForEquipment(user.rod, user.line, user.hook, user.glove, ring ? ring.ring_type : null, ring ? ringInfo.rating >= 5 : null, user.swivel);
    // create & send embed
    let author = mentionedUser || msg.author;
    let options = {
        author: [`${author.tag} (Lvl. ${user.level})`, author.displayAvatarURL()],
