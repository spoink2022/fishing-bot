...
module.exports.sendInit = async function(msg) {
    db.users.initializeAccount(msg.author.id, function() {
        msg.channel.send(`${msg.author.username}, congratulations, you've made an account!\nFor help, simply type \`${PREFIX}help\``);
    });
}
...
async function sendLeaderboards(msg) {
    if(msg.channel.type === 'dm') {
        msg.channel.send(`${msg.author.username}, this command can only be used in servers!`);
        return;
    }
    let guild = msg.guild;
    if(!guild.available) {
        msg.channel.send(`\n${msg.author.username}, a server outage is preventing us from accessing your server\'s information\nPlease try again later`);
        return;
    }
    guild.members.fetch().then(async members => {
        ...
        const namePrefixes = [':first_place:', ':second_place:', ':third_place:', ':military_medal:', ':military_medal:'];
        for(let i=0; i<Math.min(weights.length, 20); i++) {
            let weight = weights[i];
            let member = members.get(weight.userid);
            let namePrefix = i<5 ? namePrefixes[i] : `\`#${i+1}\``;
