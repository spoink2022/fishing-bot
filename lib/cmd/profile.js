...
module.exports.sendBaitCommand = async function(interaction, user) {
    ...
    if (user.level < 10) { return interaction.reply('You must reach **Lvl. 10** before you can use baits!'); }
    
    // Step 2 - Fetch Data
    let descriptionArr = [];
    const baits = await db.bait.fetchAllBaits(user.userid);
    for (const [key, value] of Object.entries(baits)) {
        if (key !== 'userid' && value !== 0) {
            descriptionArr.push(`- ${key.replace(/_/g, ' ')} **x${value}**`);
        }
    }
...
module.exports.sendEquipmentCommand = async function(interaction, user, mentionedUser) {
    ...
    interaction.reply({ embeds: [embed], files: [attachment] }).catch(err => handleMessageReplyError(err));
}