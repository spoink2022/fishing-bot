...
async function sendAllSkins(msg, user) {
    ...
    let embedFields = [];
    if (counts.equipment_banner > 0) { embedFields.push({ name: `Equipment Banners (${counts.equipment_banner}/5)`, value: embedFieldsObj.equipment_banner }) };
...
async function sendGiveSkin(msg, args, user) {
    ...
    if (!mentionedUserAcc) {
        ...
        return;
    } else if (await db.cosmetics.getUserCosmeticCategoryCount(mentionedUser.id, category) >= 5) {
        attemptReply(msg, `That user already has 5 skins in the **${categoryData.name}** category!`);
        return;
    }

    // give skin
    db.cosmetics.setOwner(skin.id, mentionedUser.id);
