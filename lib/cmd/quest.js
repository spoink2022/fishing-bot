...
module.exports.sendQuestCheckCommand = async function(interaction, user) {
    // QUEST
...
module.exports.sendQuestClaimCommand = async function(interaction, user) {
    ...
    // Step 2 - Generate Quest (if needed)
    if (user.quest_type === -1) {
        const clan = await db.clan.fetchClan(user.clan);
        let rewardMultiplier = 1;
        if (clan && clan.quest_mba) {
    ...
    // Step 3 - Validate Quest is Claimable
    if (user.quest_progress !== user.quest_requirement) {
        ...
        description: 'Check your quest with `/quest check`!'
        }
        return interaction.reply({ embeds: [embed] });
    }

    // Step 4 - Update Database + Generate New Quest
...
module.exports.sendQuestResetCommand = async function(interaction, user) {
    ...
    // Step 3 - Validate Quest is Resettable
    if (user.quest_progress === user.quest_requirement) {
        let embed = {
            color: logic.color.STATIC.default,
        ...
        description: 'Claim your quest reward with `/quest claim`!'
        }
