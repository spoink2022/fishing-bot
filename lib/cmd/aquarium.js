...
module.exports.sendScoresCommand = async function(interaction, user, locationId, mentionedUser) {
    ...
    let FishData = api.fish.getFishDataFromLocation(locationId);
    FishData.sort((a, b) => a.sizeClass - b.sizeClass + (a.legendary ? 5 : 0) - (b.legendary ? 5 : 0));
...
:globe_with_meridians: Global Location Rank: **${logic.text.numToRank(LocationRank)}**`,
        author: {
            name: `${mentionedUser.username} (Lvl. ${user.level})`,
...
module.exports.sendAquariumsCommand = async function(interaction, user, mentionedUser) {
    ...
    for (let i=0; i<LocationData.length; i++) {
        FishEntries.push({values: [], chances: []});
    }
    const capacity = Math.floor(AquariumData.max * (1+capacityMultiplier/100));
    ...
    for (let fishObj of FishData) {
        FishEntries[fishObj.location-1].values.push(FishValues[fishObj.name]);
        FishEntries[fishObj.location-1].chances.push(fishObj.chance);
    }
    for (let i=0; i<LocationData.length; i++) {
        const location = LocationData[i];

        const rawEarnRate = calculateRawEarnRate(
            FishEntries[i].values,
            FishEntries[i].chances
        );
        const earnRate = rawEarnRate * AquariumData.multiplier;
...
module.exports.sendCollectCommand = async function(interaction, user) {
    ...
    // Create quest embed
    let questString = '';
    if (user.quest_type === 3) {
        let questComplete = user.quest_requirement - user.quest_progress <= totalCoinsCollected;
        questString = questComplete ? '\nQuest Complete! (Type `/quest` to claim your rewards)' : `\nCounts towards quest! (${user.quest_progress + totalCoinsCollected}/${user.quest_requirement} coins)`;
    }
    ...
    let embedArr = [{
        ...
        description: `You now have ${user.coins + totalCoinsCollected} coins!${questString}`,
    }];
    sendReply(interaction, { embeds: embedArr }).catch(err => handleMessageReplyError(err));
}