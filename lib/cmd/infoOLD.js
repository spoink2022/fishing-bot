...
const { createItemShowcaseCanvas, createLineShowcaseCanvas, createFishShowcaseCanvas, createBaitShowcaseCanvas, createBackgroundShowcaseCanvas, createCroppedItemShowcaseCanvas, createSkinShowcaseCanvas, createSkinHelpCanvas } = require('../misc/canvas.js');
...
async function sendItemInfo(msg, args, user) {
    ...
    if(['rod', 'rods', 'line', 'lines', 'hook', 'hooks', 'glove', 'gloves', 'swivel', 'swivels'].includes(itemName)) {
        ...
        for(const item of allItemInfo) {
            ...
            options.fields[Math.floor(itemCount/35)].value += `\n${capitalizeWords(item.name)} ${equipmentPiece}`;
        }
...
async function sendEvent(msg) {
    ...
    if(!eventEntry) {
        ...
        // add image for specific events
        if (eventEntry.type === 'skinfest') {
            let params = eventEntry.params.split(' ');
            let canvasBuffer = await createSkinShowcaseCanvas(IMAGES.cosmetics[params[0]][params[1]], 600, 450);
            options.attachment = { name: 'skin.png', content: canvasBuffer }
        }
    }
    let embed = await createEmbed(options);
    msg.channel.send(embed);
