...
module.exports.sendSetLocationCommand = async function(interaction, user, locationId) {
    ...
    if (!locationId && locationId !== 0) { return interaction.reply('You must specify the location to set to!'); } // text-based command calls
    ...
    if (user.location === locationId) { return interaction.reply(`Location ${locationId} is already your current location!`); }
...
module.exports.sendLocationCommand = async function(interaction, user, locationId) {
    // LOCATION
    // Step 1 - Validate Id Argument
    if (!locationId) { return interaction.reply('You must specify the id (positive number) of the location to view!'); } // text-based command calls
    if (locationId <= 0) { return interaction.reply(`**${locationId}** is not a valid location id!`); }
    const LocationData = api.fish.getLocationData(locationId);
    if (!LocationData) { return interaction.reply(`**${locationId}** is not a valid location id!`); }
    if (user.level < LocationData.level - 10) { return interaction.reply('You must be a higher level to view the information of this location!'); }
