...
${user.big_supporter > 1 ? `\n:label: ${user.big_supporter - 1} Extra Premium Supporter${user.big_supporter !== 1 ? 's' : ''} (see \`giftpremium\` command)` : ''}\
${user.custom_fish > 0 ? `\n:scroll: ${user.custom_fish} Custom Fish Server Gift${user.custom_fish !== 1 ? 's' : ''} (see \`redeem\` command)` : ''}`
    };
