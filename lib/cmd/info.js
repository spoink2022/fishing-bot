...
const CATEGORIES = [
    ...
    'glove', 'gloves', 'swivel', 'swivels', 'hull', 'hulls', 'engine', 'engines', 'container', 'containers', 'propeller', 'propellers'
];
...
module.exports.sendInfoCommand = async function(interaction, user, option) {
    ...
    if (last === 'ring') {
        ...
        if (!api.equipment.getRingNames().includes(ringName)) { return interaction.reply(`**${option}** is not a valid ring type!`); }
        ...
        if (!AquariumNames[aquariumId]) { return interaction.reply(`**${option}** is not a valid aquarium upgrade!`); }
        ...
        if (!RodNames[rodId]) { return interaction.reply(`**${option}** is not a valid fishing rod!`); }
        ...
        if (!LineNames[lineId]) { return interaction.reply(`**${option}** is not a valid fishing line!`); }
        ...
        if (!HookNames[hookId]) { return interaction.reply(`**${option}** is not a valid hook!`); }
        ...
        if (!GloveNames[gloveId]) { return interaction.reply(`**${option}** is not a valid glove!`); }
        ...
        if (!SwivelNames[swivelId]) { return interaction.reply(`**${option}** is not a valid swivel!`); }
        ...
        if (!HullNames[hullId]) { return interaction.reply(`**${option}** is not a valid hull!`); }
        ...
        if (!EngineNames[engineId]) { return interaction.reply(`**${option}** is not a valid engine!`); }
        ...
        if (!ContainerNames[containerId]) { return interaction.reply(`**${option}** is not a valid container!`); }
        ...
        if (!PropellerNames[propellerId]) { return interaction.reply(`**${option}** is not a valid propeller!`); }
...
async function createSpecificEngineEmbed(engineId) {
    ...
    return [{
        ...
        description: `:fuelpump: Fuel Efficiency: ${EngineData.efficiency}km/unit
...
async function createSpecificPropellerEmbed(propellerId) {
    ...
    return [{
        ...
        description: `:alarm_clock: Cooldown: ${logic.text.millisToDaysAndHours(PropellerData.cooldown)}
...
\`hulls\` \`engines\` \`containers\` \`propellers\``
            },
            {
                name: '/info [thing]',
...
function createCategoryInfoEmbed(category, author, user) {
    ...
    switch (category) {
        ...
        case 'swivel':
            ...
            break;
        case 'hull':
            embed = createHullCategoryEmbed(user);
            break;
        case 'engine':
            embed = createEngineCategoryEmbed(user);
            break;
        case 'container':
            embed = createContainerCategoryEmbed(user);
            break;
        case 'propeller':
            embed = createPropellerCategoryEmbed(user);
            break;
        default:
            break;
    }
