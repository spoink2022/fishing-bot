...
async function sendFishInfo(msg, args, user) {
    ...
    for(let entry of locationInfo.fish) { if(entry.id===index) { rarity = percentToRarity(entry.chance).toLowerCase(); }}
...
async function sendHelp(msg) {
    let options = {
        ...
        **Helpful Links**\n\
        ...
        Commands can also be found by typing `.info`'
    }
    let embed = await createEmbed(options);
    msg.channel.send(embed);
}
