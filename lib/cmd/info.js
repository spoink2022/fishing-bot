...
\n\n:one: Fish with `.fish` - this command has a 75 minute cooldown.\
\n:two: Check your aquarium with `.aquarium` - coins can be collected from here.\
\n:three: Vote with `.vote` to reset your fishing cooldown.\
\n:four: See your equipment with `.equipment` - upgrade with `.shop`\
\n:five: Use `.help` to for access to a help menu.\
\n\n**Relevant Guides**\
\nAquarium\
\nTier\
\nStats\
...
\n\n:one: Collect aquarium money directly with `.collect`\
\n:two: Access a commands list with `.cmd`\
\n:three: Use `.cmd [command]` to see command shortcuts\
\n:four: For anything you\'re unsure of, check the help menu!\
\n\n**Relevant Guides**\
\nInfo\
\nWeather\
...
\n\n:one: Change locations with `.setlocation 2` - `.sl` for short.\
\n:two: Type `.quest` to view, claim rewards for, and reset your quest.\
\n:three: See the weekly bounty with `.bounty`\
\n:four: Use `.baitshop` to spend your :lollipop: from quests and bounties.\
\n:six: Regularly check `.event`\
\n\n**Relevant Guides**\
\nBait\
\nQuest\
...
\n:two: Use `.cards` & `.card [ID]` to view cards.\
\n:three: Use `.rings` & `.ring [ID]` to view rings.\
\n:four: Your level is now high enough to create a clan.\
\n\n**Relevant Guides**\
\nCard\
\nRing\
\n\n**Make sure you check out the guides! Things are getting complex...**'
        }
    } else if (args[0] === '5') {
        var options = {
...
\n\n**Location Info**\
\nType `.li` to pull up a list of available locations and their weather.\
\nType `.li [ID]` to view information about a specific location.\
\n\n**Fish Info**\
\nType `.fi` to pull up a list of all fish families.\
\nType `.fi [family]` to view all unlocked fish of a given family.\
\nType `.fi [fish]` to view information about a specific fish.\
\n\n**Item Info**\
\nType `.ii [equipment type]` to view all items of a certain type.\
\n[equipment type] must be one of: rod, hook, line, aquarium, glove, ring, swivel, bait.\
\nType `.ii [item name]` to view a specific equipment piece or item.'
        }
    } else if (args[0] === 'tier') {
        var options = {
...
\nView with `.aquarium [ID]`\
\nView other fisher\'s aquariums with `.aquarium [ID] [@user]`\
\n\n**Aquariums generate money**\
\nCollect from ALL aquariums with `.collect`\
...
\nWeather for all locations can be viewed with `.li`\
...
\n\n**Quests**\
\nThe `.quest` command is used to view, reset, and claim rewards for a quest.\
\nAfter claiming a quest reward, type `.quest` to view your next quest.\
\nQuests can be reset after 3 days.\
\n\n**Bounties**\
\nThe `.bounty` command is used to view the weekly bounty.\
...
\nEach week\'s bounty can only be completed once.'
    ...
    \nBaits are purchased with `.baitshop` - only buy good deals!\
    \nThe bait shop refreshes daily.\
    \nTo view information about a bait, use `.ii [bait]`\
    \nView your baits with `.bait`\
    \n\n**Using Bait**\
    \nType `.fish [bait]` to use a bait.\
    \nUsing `.fish` will assume no bait as usual.'
            }
    } else if (args[0] === 'clan') {
        var options = {
...
\n`.shop` - Buy a ring pack (1 ring).\
\n`.equipment` - See a simplified view of your equipped ring.\
\n`.ring` - View your rings.\
\n`.ring [ID]` - View, equip, or sell a ring. :white_check_mark: :moneybag:\
\n`.ii rings` - View all ring types.\
\n`.ii [ring type]` - View a ring type.\
...
\n\n**Viewing Cards**\
\nType `.card` to view your cards.\
\nType `.card [ID]` to view a specific card.\
\n\n**Using Cards**\
...
\nUse `.givecard [ID] [@user]` to give cards.'
        }
...
\n`.guide 2` - Getting Started (Continued)\
\n`.guide 3` - Level 10+\
\n`.guide 4` - Level 20+\
\n`.guide 5` - Level 50+\
\n\n:nerd: **Concepts**\
\n`.help stats` - Viewing Player Data\
\n`.help info` - Viewing Game Data\
\n`.help tier` - Tiers Explained\
\n`.help aquarium` - Aquariums Explained\
\n`.help weather` - A Guide to Weather\
\n\n:star2: **Features**\
\n`.help quest` - Quests & Bounties\
\n`.help bait` - Everything About Baits\
\n`.help clan` - Everything About Clans\
\n`.help ring` - A Special Equipment Type\
\n`.help card` - Collecting Cards\
\n\n**Helpful Links**\
