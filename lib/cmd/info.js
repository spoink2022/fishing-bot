...
const { kgToWeight } = require('../logic/text');
...
const CATEGORIES = [
    'fish', 'bait', 'baits', 'chum', 'ring', 'rings', 'aquarium', 'aquariums', 'rod', 'rods', 'line', 'lines', 'hook', 'hooks', 
...
module.exports.sendInfoCommand = async function(interaction, user, option) {
    ...
    } else if (api.bait.getAllChumBaitNames().includes(arg.replace(/ /g, '_'))) {
        [embed, attachment] = await createSpecificChumBaitEmbed(arg.replace(/ /g, '_'));
    ...
    embed.author = {
        name: `${interaction.user.username} (Lvl. ${user.level})`,
...
async function createSpecificBaitEmbed(baitName) {
    ...
    return [{
        ...
        footer: {
            text: BaitData.bonus ? '*Weight buff applied after swivel and glove buffs' : ''
        }
    }, new MessageAttachment(canvasBuffer, `${baitName}.png`)]
}

async function createSpecificChumBaitEmbed(baitName) {
    const ChumBaitData = api.bait.getChumBaitData(baitName);
    const canvasBuffer = await createInfoCanvas('chum_bait', baitName);
    let bonusDetails = `Estimated Value: **${ChumBaitData.value}** :lollipop:`;
    if (ChumBaitData.tier !== 'D') { bonusDetails += `\n- Minimum fish tier: **${ChumBaitData.tier}**`; }
    if (ChumBaitData.coin_exp) { bonusDetails += `\n - Coin/Exp increase by **${ChumBaitData.coin_exp}%**`; }
    if (ChumBaitData.sizes.length !== 4) { bonusDetails += `\n- Catches exclusively **${ChumBaitData.sizes.map(num => SIZE_CLASSES[num - 1]).join(', ')}** fish`; }
    if (ChumBaitData.bonus) { bonusDetails += `\n - Temporary max weight increase by **${ChumBaitData.bonus}%**\n- Weight buff applied *after* swivel and glove buffs`; }
    return [{
        title: `Info - ${capitalize(baitName.replace(/_/g, ' '))} (Chum Bait)`,
        description: `${bonusDetails}`,
        image: { url: `attachment://${baitName}.png` },
        footer: {
            text: ChumBaitData.bonus ? '*Weight buff applied after swivel and glove buffs' : ''
        }
    }, new MessageAttachment(canvasBuffer, `${baitName}.png`)]
}
...
async function createSpecificRingEmbed(ringType) {
    ...
    if (RingData.dropMultiplier && RingData.dropMultiplier == 0.3) { embedFields[0].value += `**significant** decrease in card drop rate\n`; }
...
:four_leaf_clover: Activation Chance: ${GloveData.chance}%\n:mechanical_arm: Activation Bonus: +${kgToWeight(GloveData.bonus)}`,
...
:shark: Activation Bonus: +${kgToWeight(SwivelData.bonus)}`,
        ...
        fields: ['common', 'uncommon', 'rare'].map(rarity => {
            return {
                ...
                value: [...new Set(ClanLocationData.spawns[rarity])].map(s => {
                    ...
                    else if (s[0] === 'b' && parseInt(s[1]) <= 4) { return `${s.substring(1)}-star bait` }
                    else if (s[0] === 'b' && parseInt(s[1]) > 4) { return `${['Spiked', 'Ultimate', 'Silver', 'Gold', 'Diamond'][parseInt(s[1]) - 5]} chum` }
                }).join('\n')
            }
        }),
...
Categories:
\`fish\`, \`bait\`, \`chum\`, \`rings\`, \`aquariums\`, 
...
function createBaitCategoryEmbed() {
    ...
    for (const[key, value] of Object.entries(AllBaitData)) {
        embedFields[value.banned ? 4 : value.stars - 1].value += `*${capitalize(key.replace(/_/g, ' '))}*\n`;
    }
    return {
        title: 'Info - Bait',
        description: `*For info on chum baits, see \`/info chum\`*\
\n\n:star2: Baits unlock at Lvl. 10
Use baits while fishing for better results.
For info on baits use \`/help bait\`
For specific info on a bait, use \`/info [Bait Name]\`
\nBelow, baits are sorted by value.`,
        fields: embedFields
    };
}
function createChumCategoryEmbed() {
    const AllChumBaitData = api.bait.getAllChumBaitData();
    let embedFields = [
        { name: 'Cheap', value: '\u200b', inline: true },
        { name: 'Ultimate', value: '\u200b', inline: true },
        { name: '\u200b', value: '\u200b', inline: true},
        { name: 'Standard', value: '\u200b', inline: true },
        { name: 'Legendary', value: '\u200b', inline: true },
        { name: '\u200b', value: '\u200b', inline: true},
        { name: 'Spiked', value: '\u200b', inline: true }
    ];
    for (const [key, value] of Object.entries(AllChumBaitData)) {
        embedFields[(value.stars-1)*3 - Math.floor(value.stars/4)*8].value += `*${capitalize(key.replace(/_/g, ' '))}*\n`;
    }
    return {
        title: 'Info - Chum Bait',
        description: ':star2: Chum Baits unlock at Lvl. 40\
\nUse chum baits while fishing for better results.\
\nFor info on chum baits use \`/help chum\`\
\nFor specific info on a chum bait, use \`/info [Chum Bait Name]\`\
\n\nBelow, chum baits are grouped by value.',
        fields: embedFields
    }
}
...
function createLineCategoryEmbed(user) {
    ...
    const columnSize = Math.ceil(UnlockedLineNames.length / 3);
    let embedFields = [{ name: 'Relevant Lines', value: '\u200b', inline: true }, { name: '\u200b', value: '\u200b', inline: true }, { name: '\u200b', value: '\u200b', inline: true }];
...
function createHookCategoryEmbed(user) {
    ...
    const columnSize = Math.ceil(UnlockedHookNames.length / 3);
    let embedFields = [{ name: 'Relevant Hooks', value: '\u200b', inline: true }, { name: '\u200b', value: '\u200b', inline: true }, { name: '\u200b', value: '\u200b', inline: true }];
