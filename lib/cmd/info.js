...
module.exports.sendInfoCommand = async function(interaction, user, option) {
    ...
    if (last === 'ring') {
        ...
        if (!AllClanLocationData[clanLocationId]) { return interaction.reply(`**${option}** is not a valid clan location number!`); }
...
function createCategoryInfoEmbed(category, author, user) {
    ...
    switch (category) {
        ...
        case 'propeller':
            ...
            break;
        case 'clanlocation':
            embed = createClanLocationCategoryEmbed(user);
            break;
        default:
            break;
    }
...
function createClanLocationCategoryEmbed(user) {
    const AllClanLocationData = api.clan.getAllLocationData();
    return {
