...
module.exports.sendInfoCommand = async function(interaction, user, option) {
    ...
    if (arg.endsWith(' ring')) {
        ...
        const gloveId = api.equipment.getGloveNames().indexOf(capitalize(arg).split(' ').slice(0, -1).join(' '));
        ...
        const swivelId = api.equipment.getSwivelNames().indexOf(capitalize(arg).slice(0, -7));
    ...
    } else if (arg.endsWith(' hull')) {
        const hullId = api.clan.getAllHullData().map(obj => obj.name).indexOf((arg).slice(0, -5));
        if (hullId === -1) { return interaction.reply(`**${option}** is not a valid hull!`); }
        [embed, attachment] = await createSpecificHullEmbed(hullId + 1);
    } else if (arg.endsWith(' engine')) {
        const engineId = api.clan.getAllEngineData().map(obj => obj.name).indexOf((arg).slice(0, -7));
        if (engineId === -1) { return interaction.reply(`**${option}** is not a valid engine!`); }
        [embed, attachment] = await createSpecificEngineEmbed(engineId + 1);
    } else if (arg.endsWith(' container')) {
        const containerId = api.clan.getAllContainerData().map(obj => obj.name).indexOf((arg).slice(0, -10));
        if (containerId === -1) { return interaction.reply(`**${option}** is not a valid container!`); }
        [embed, attachment] = await createSpecificContainerEmbed(containerId + 1);
    } else if (arg.endsWith(' propeller')) {
        const propellerId = api.clan.getAllPropellerData().map(obj => obj.name).indexOf((arg).slice(0, -10));
        if (propellerId === -1) { return interaction.reply(`**${option}** is not a valid propeller!`); }
