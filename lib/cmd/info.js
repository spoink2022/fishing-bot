...
async function sendItemInfo(msg, args, user) {
    ...
    if(['rod', 'rods', 'line', 'lines', 'hook', 'hooks'].includes(itemName)) {
        ...
        else if(index > api.fishing.getHighestItemID('rods', user.level)) { msg.reply(`You must be a higher level to view this item!`); return; }
        ...
        else if(index > api.fishing.getHighestItemID('lines', user.level)) { msg.reply(`You must be a higher level to view this item!`); return; }
        ...
        else if(index > api.fishing.getHighestItemID('hooks', user.level)) { msg.reply(`You must be a higher level to view this item!`); return; }
        ...
        else if(index > api.gamedata.getHighestAquariumID(user.level)) { msg.reply(`You must be a higher level to view this item!`); return; }
...
async function sendFishInfo(msg, args, user) {
    ...
    if(!api.fishing.getUnlockedLocations(user.level).includes(fishInfo.location)) {
        msg.reply('You may only view the information of fish from locations you have unlocked!');
        return;
    }
...
async function sendLocationInfo(msg, args, user) {
    ...
    if(!locationInfo) {
        ...
        msg.reply(`You need to have unlocked a location to view its information!`);
        return;
    }
    let fishArr = [];
