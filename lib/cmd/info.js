...
async function sendItemInfo(msg, args, user) {
    ...
    if(['rod', 'rods', 'line', 'lines', 'hook', 'hooks'].includes(itemName)) {
        ...
        if(index === -1) { msg.channel.send(`${msg.author.username}, \`${itemName}\` is not a valid rod!`); return; }
        else if(index > api.fishing.getHighestItemID('rods', user.level)) { msg.channel.send(`${msg.author.username}, you must be a higher level to view this item!`); return; }
        ...
        if(index === -1) { msg.channel.send(`${msg.author.username}, \`${itemName}\` is not a valid line!`); return; }
        else if(index > api.fishing.getHighestItemID('lines', user.level)) { msg.channel.send(`${msg.author.username}, you must be a higher level to view this item!`); return; }
        ...
        if(index === -1) { msg.channel.send(`${msg.author.username}, \`${itemName}\` is not a valid hook!`); return; }
        else if(index > api.fishing.getHighestItemID('hooks', user.level)) { msg.channel.send(`${msg.author.username}, you must be a higher level to view this item!`); return; }
        ...
        if(index === -1) { msg.channel.send(`${msg.author.username}, \`${itemName}\` is not a valid aquarium!`); return; }
        else if(index > api.gamedata.getHighestAquariumID(user.level)) { msg.channel.send(`${msg.author.username}, you must be a higher level to view this item!`); return; }
        ...
        msg.channel.send(`${msg.author.username}, \`${itemName || ' '}\` is not a valid item!`);
        return;
    }
...
async function sendFishInfo(msg, args, user) {
    ...
    if(!fishNames.includes(fishName)) {
        msg.channel.send(`${msg.author.username}, \`${fishName || ' '}\` is not a valid fish!`);
        return;
    }
    ...
    if(!api.fishing.getUnlockedLocations(user.level).includes(fishInfo.location)) {
        msg.channel.send(`${msg.author.username}, you may only view the information of fish from locations you have unlocked!`);
        return;
    }
...
async function sendLocationInfo(msg, args, user) {
    ...
    if(!locationInfo) {
        msg.channel.send(`${msg.author.username}, \`${args.join(' ') || ' '}\` is not a valid location ID!`);
        return;
    } else if(!api.fishing.getUnlockedLocations(user.level).includes(locationInfo.id)) {
        msg.channel.send(`${msg.author.username}, you need to have unlocked a location to view its information!`);
        return;
    }
    let fishArr = [];
