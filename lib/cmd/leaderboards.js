...
module.exports.sendLeaderboardsCommand = async function(interaction, user, category) {
    ...
    .map(guildMember => {
        tags[guildMember.user.id] = `${guildMember.user.username}`;
...
module.exports.sendFishLeaderboardsCommand = async function(interaction, user, option) {
    ...
    .filter(guildMember => !guildMember.user.bot)
    .map(guildMember => {
        tags[guildMember.user.id] = `${guildMember.user.username.replace(/\*/g, '\\*').replace(/_/g, '\\_').replace(/~/g, '\\~').replace(/`/g, '\\`')}`;
        return guildMember.user.id;
    });
...
${globalRank ? (globalRank.rank < 10 ? RANK_EMOJIS[globalRank.rank - 1] : `\`#${globalRank.rank}\``) : '\`N.A\`'}\
 ${interaction.user.username} - ${globalRank ? `${logic.text.kgToWeight(calculateFishWeight(globalRank.r, FishData))} (${logic.text.rToTier(globalRank.r)})` : 'No Catch'}`;
...
${guildRank ? (guildRank.rank < 10 ? RANK_EMOJIS[guildRank.rank - 1] : `\`#${guildRank.rank}\``) : '\`N.A\`'}\
 ${interaction.user.username} - ${guildRank ? `${logic.text.kgToWeight(calculateFishWeight(guildRank.r, FishData))} (${logic.text.rToTier(guildRank.r)})` : 'No Catch'}`
