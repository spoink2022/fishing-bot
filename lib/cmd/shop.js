...
${server.premium_tier === 9 ? '' : 'Redeem to this server again to unlock another deal.\n'}\u200b`,
    ...
    sendReply(interaction, { embeds: embedArr, components: rows }).then(async (sentMessage) => {
        ...
        collector.on('end', (collected, reason) => {
            if (reason === 'time') {
                if (interaction.constructor.name === 'Message') { return sentMessage.edit({ embeds: embedArr, components: [] }).catch(err => {}); }
...
module.exports.sendShopCommand = async function(interaction, user) {
    ...
    if (user.level >= 50 && !user.boat_swivel) { embedFields.push(createShopField(user, { level: 50, price: 10000, name: 'Swivel' }, `Swivel Extension (Boat) ${api.emoji.CLANBOAT_HULL}`, 'Extension Boat')); }
    if (user.level >= 100 && !user.whale_swivel) { embedFields.push(createShopField(user, { level: 100, price: 500000, name: 'Swivel' }, 'Swivel Extension (Whales)  :whale2:', 'Extension Whale')); }
    if (user.level >= 140 && !user.prestige_swivel) { embedFields.push(createShopField(user, {level: 140, price: 10000000, name: 'Swivel'}, 'Swivel Extension (Prestige) :dna:', 'Extension Prestige')) }
