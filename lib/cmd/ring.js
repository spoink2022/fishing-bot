...
module.exports.sendRingCommand = async function(interaction, user, ringId, mentionedUser) {
    ...
    if (!ringId) { return interaction.reply('You must specify the ID of the ring!'); } // text-based command calls
...
${ring.id == user.ring ? ':white_check_mark:' : ':no_entry_sign: Not'} Equipped`,
    ...
    if (!USER_MENTIONED) {
        ...
        rowArr = [new MessageActionRow().addComponents(equipButton, sellButton)];
    }
