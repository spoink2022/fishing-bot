...
async function sendFish(msg, user) {
    ...
    let res = await createCanvasForFishingPre(user, possibleFish);
    ...
    await msg.channel.send(embed).then(async (sentEmbed) => {
        ...
        if(fish.tier !== 'f' && !lineSnapped) {
            ...
            if(!oldAquariumEntry || oldAquariumEntry < fish.sizeMult) {
                ...
                let maxAquariumEarnings = api.gamedata.getAquariumInfo(user.aquarium_level).max;
                let aquariumEarnings = Math.min(maxAquariumEarnings, await gameLogic.getAquariumEarnings(user.location, user));
...
async function sendAquarium(msg, args, user) {
    ...
    let canvasBuffer = await createCanvasForAquarium(locationID, user.aquarium_level, fishContents  );
    ...
    for(const[key, val] of Object.entries(fishContents)) {
        if(val) {
            ...
            embedFieldVal += `**(${val.tier.toUpperCase()})** ${capitalizeWords(key.replace('_', ' '))} - ${weightStr}\n`;
        }
        totalFish++;
    }
    // creating embed
    let options = {
        author: [`${msg.author.tag} (Lvl. ${user.level})`, msg.author.displayAvatarURL()],
        title: `Aquarium - ${LocationInfo.name} (${earnRate}/hr)`,
        description: `Coins Earned: ${earnings}/${AquariumInfo.max} :coin:`,
...
async function sendCollect(msg, args, user) {
    ...
    let options = {
        author: [`${msg.author.tag} (Lvl. ${user.level})`, msg.author.displayAvatarURL()],
        title: `You collected ${toCollect} coins! :coin:`
    };
    db.users.collectAquariumEarnings(msg.author.id, toCollect);
    let embed = await createEmbed(options);
    msg.channel.send(embed);
}
...
async function sendShop(msg, user) {
    ...
    msg.channel.send(embed).then(async(sentEmbed) => {
        ...
        collector.on('collect',async(reactionData) => {
            ...
            let options = {
                author: [`${msg.author.tag} (Lvl. ${user.level})`, msg.author.displayAvatarURL()],
                title: `Successfully Purchased Item ${reactionData.emoji.name[0]}!`
            };
            let embed = await createEmbed(options);
            msg.channel.send(embed);
        });
