...
async function sendFish(msg, args, user) {
    ...
    await msg.channel.send(embed).then(async (sentEmbed) => {
        ...
        let clanBonus = clan && gameLogic.getStarCount(clan.fish_caught) >= 6 ? 1.1 : 1;
        const lineSnapped = fish.weight > (Math.min(lineInfo.maxWeight, hookInfo.maxWeight, rodInfo.maxWeight) + gloveBonus + swivelBonus) * clanBonus;
        ...
        let bonusExp = 0;
        if (clan && gameLogic.getStarCount(clan.fish_caught) >= 3) { // CLAN 3-STAR => +10% exp
            bonusExp = Math.max(Math.ceil(expGained * 0.1), 1);
        }
        // apply coin & exp modifiers
        coinsGained *= hookInfo.multiplier;
        coinsGained = fish.tier === 'f' ? 0 : Math.max(Math.ceil(coinsGained), 2);
        let bonusCoins = 0;
        if (clan && gameLogic.getStarCount(clan.fish_caught) >= 2) { // CLAN 2-STAR => +5% coins
            bonusCoins = Math.ceil(coinsGained * 0.05);
        }
        ...
        if (bonusCoins) { descriptionSuffix += `\nGained ${bonusCoins} bonus coins! :coin:`; }
...
async function sendAquarium(msg, args, user) {
    ...
    let clan = clanMember ? await db.clan.fetchClan(clanMember.clan) : null;
    let aquariumCapacity = Math.floor(AquariumInfo.max * (clan && gameLogic.getStarCount(clan.fish_caught) >= 5 ? 1.2 : 1));
...
async function unpackRing(msg, user, packType, packInfo) {
    ...
    if (rings.length >= 10) {
        msg.channel.send('You already have 10 rings! Sell one of them to purchase more!');
        return;
    }
...
async function sendQuest(msg, user) {
    ...
    let clanMember = await db.clan.fetchMember(msg.author.id);
    let clan = clanMember ? await db.clan.fetchClan(clanMember.clan) : null;
    let reducedCooldown = clan && gameLogic.getStarCount(clan.fish_caught) >= 4;
    let questCooldown = reducedCooldown ? 1000*60*60*24 : 1000*60*60*24*3;
    ...
    if (user.quest.progress === user.quest.qt) {
        ...
        options.description = `You will be able to reset your quest when it is ${reducedCooldown ? '1 day' : '3 days'} old.`;
    }
    let embed = await createEmbed(options);
    msg.channel.send(embed).then(async (sentEmbed) => {
        if(user.quest && user.quest.progress === user.quest.qt) {
...
async function sendRing(msg, args, user) {
    if (user.level < 20) { msg.channel.send('You must reach level 20 before you may access rings!'); return; }
