...
async function sendFish(msg, args, user) {
    ...
    await msg.channel.send(embed).then(async (sentEmbed) => {
        ...
        let clanBonus = clan && gameLogic.getStarCount(clan.fish_caught) >= 6 ? 1.05 : 1;
        ...
        let bonusExp = 0;
        if (clan && gameLogic.getStarCount(clan.fish_caught) >= 3) { // CLAN 3-STAR => +5% exp
            bonusExp = Math.max(Math.ceil(expGained * 0.05), 1);
        }
...
async function sendAquarium(msg, args, user) {
    ...
    let clan = clanMember ? await db.clan.fetchClan(clanMember.clan) : null;
    let aquariumCapacity = Math.floor(AquariumInfo.max * (clan && gameLogic.getStarCount(clan.fish_caught) >= 5 ? 1.1 : 1));
