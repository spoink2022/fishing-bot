...
const { createCanvasForFishingPre, createCanvasForFishingPost, createCanvasForAquarium, createCanvasForBounty } = require('../misc/canvas.js');
...
async function sendFish(msg, args, user) {
    ...
    await msg.channel.send(embed).then(async (sentEmbed) => {
        ...
        const rodInfo = api.fishing.getRodData(user.rod);
        const lineSnapped = fish.weight > Math.min(lineInfo.maxWeight, hookInfo.maxWeight, rodInfo.maxWeight) + gloveBonus;
