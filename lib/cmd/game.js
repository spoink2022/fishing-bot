...
async function sendFish(msg, user) {
    ...
    let res = await createCanvasForFishingPre(user, possibleFish, locationInfo.zoom, rodInfo.size);
...
async function sendAquarium(msg, args, user) {
    ...
    if(!locationID || !LocationInfo) {
        msg.channel.send(`${msg.author.username}, \`${args[0]}\` is not a valid location ID!`);
        return;
    } else if(user.level < LocationInfo.level) {
        msg.channel.send(`${msg.author.username}, you have not unlocked this location yet!`);
        return;
    }
...
async function sendCollect(msg, args, user) {
    ...
    const LocationInfo = api.fishing.getLocationData(locationID);
    if(!locationID || !LocationInfo) {
        msg.channel.send(`${msg.author.username}, \`${locationID}\` is not a valid location ID!`);
        return;
    } else if(user.level < LocationInfo.level) {
        msg.channel.send(`${msg.author.username}, you have not unlocked this location yet!`);
        return;
    ...
    let earnings = await gameLogic.getAquariumEarnings(locationID, user);
    if(earnings < 1) {
        msg.channel.send(`${msg.author.username}, you must have at least 1 coin in an aquarium to collect!`);
        return; 
    }
...
async function sendCollectAll(msg, user) {
    ...
    if(totalEarnings === 0) {
        msg.channel.send(`${msg.author.username}, you don't have any coins in your aquariums to collect!`);
        return;
    }
...
async function sendShop(msg, user) {
    if(shoppingUsers.includes(msg.author.id)) {
        msg.channel.send(`${msg.author.username}, you already have the shop menu open!`);
        return;
    }
...
async function sendSetLocation(msg, args, user) {
    ...
    if(!locationID || !LocationInfo) {
        ...
        msg.channel.send(`${msg.author.username}, Location ${locationID} (${LocationInfo.name}) is already your current location!`);
        return;
    }
