...
async function sendFish(msg, user) {
    ...
    await msg.channel.send(embed).then(async (sentEmbed) => {
        ...
        if(fish.tier !== 'f' && !lineSnapped) {
            var oldAquariumEntry = await db.aquarium.getLargestSize(msg.author.id, user.location, fishInfo.name);
            if(!oldAquariumEntry || oldAquariumEntry < fish.sizeMult) {
                db.aquarium.setLargestSize(msg.author.id, user.location, fishInfo.name, fish.sizeMult);
        ...
        let options = {
            ...
            fields: fish.tier === 'f' ? [] : [{
                name: 'Weight :scales:',
                value: fish.weight > 1 ? `${fish.weight}kg` : `${Math.round(fish.weight*1000)}g`,
                inline: true
            }],
