...
const { capitalizeWords, convertQuestToString, parseQuestString, getIncrementedQuestString } = require('../misc/str_functions.js');
...
async function sendFish(msg, user) {
    ...
    await msg.channel.send(embed).then(async (sentEmbed) => {
        ...
        if(fish.tier !== 'f' && !lineSnapped) {
            ...
            if(user.quest.type === 'catch_tier' && user.quest.progress !== user.quest.qt) {
                if(TIER_RANKS[fish.tier] >= TIER_RANKS[user.quest.tier.toLowerCase()]) {
                    ...
                    if(user.quest.progress === user.quest.qt) { // quest complete
                        ...
                        descriptionSuffix += `\nCounts toward quest! (now ${user.quest.progress}/${user.quest.qt}) :clock${user.quest.progress%13}:`;
                    }
                }
            }
        }
...
async function sendAquarium(msg, args, user) {
    ...
    msg.channel.send(embed).then(async (sentEmbed) => {
        ...
        collector.on('end', async() => {
            const hasManageMessagesPermissions = (await msg.guild.members.fetch(client.user.id)).hasPermission('MANAGE_MESSAGES');
