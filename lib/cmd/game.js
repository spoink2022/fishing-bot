...
async function sendFish(msg, args, user) {
    ...
    let possibleFishIDs = gameLogic.getPossibleFishIDs(locationInfo.trashChance, locationInfo.fish, sizeClasses, boostedFamilies, baitUsed);
    let possibleFish = gameLogic.getPossibleFish(possibleFishIDs, minTier); // takes IDs and returns fish objects
    let res = await createCanvasForFishingPre(user, possibleFish, locationInfo.zoom, rodInfo.size, baitUsed ? baitName : false);
...
async function sendCard(msg, args, user) {
    ...
    const marketInfo = await db.misc.market.getCurrentEntry();
    ...
    let realValue = Math.round(gameLogic.getCardValue(card, card.grade)); // parameter incorporates grade
    let marketValue = Math.floor(gameLogic.getCardValue(card) * marketInfo[GRADES[card.grade-1].toLowerCase()]); // no parameter means grade not incorporated (must multiply)
    ...
    let embedDescription = [
        ...
        `:arrow_right: Recommended Value: **${realValue}** :lollipop:`,
        `:moneybag: Current Market Value: **${marketValue}** :lollipop:`
    ].join('\n');
    let options = {
        title: `Card - ${card.id}`,
        author: [`${msg.author.tag} (Lvl. ${user.level})`, msg.author.displayAvatarURL()],
        ...
        footer: `React with a moneybag to sell at market value\nReact with a truck (if it appears) to send to your aquarium`
    }
    let embed = await createEmbed(options);
    msg.channel.send(embed).then(async(sentEmbed) => {
        let currentRecord = await db.aquarium.getLargestSize(user.userid, fishInfo.name);
        let newRecord = !currentRecord || currentRecord < card.r;
        ...
        collector.on('collect', async(reactionData) => {
            ...
            if (!card) {
                ...
                db.users.updateColumn(user.userid, 'lollipops', marketValue);
                db.misc.cards.removeEntry(card.id);
                let options = {
                    author: [`${msg.author.tag} (Lvl. ${user.level})`, msg.author.displayAvatarURL()],
                    title: `Success!`,
                    color: api.visuals.getColor('classic', 'success'),
                    description: `You sold your fish card and got ${marketValue} :lollipop:`
                };
                let embed = await createEmbed(options);
                msg.channel.send(embed);
