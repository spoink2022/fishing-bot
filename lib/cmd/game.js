...
async function sendFish(msg, args, user) {
    ...
    await msg.channel.send(embed).then(async (sentEmbed) => {
        ...
        if(fish.tier !== 'f' && !lineSnapped) {
            ...
            if(user.quest && user.quest.progress < user.quest.qt) {
                if(user.quest.type === 'catch_tier') {
                    if(TIER_RANKS[fish.tier] >= TIER_RANKS[user.quest.tier.toLowerCase()]) {
                        ...
                        QUEST_COMPLETE = user.quest.progress === user.quest.qt;
                ...
                } else if(user.quest.type === 'use_bait') {
                    if (baitUsed && baitName.replace(/ /g, '_') === user.quest.name) {
                        user.quest.progress++;
                        await db.users.updateQuest(user.userid, convertQuestToString(user.quest));
                        QUEST_COMPLETE = user.quest.progress === user.quest.qt;
                        descriptionSuffix += `\nCounts toward quest! (now ${user.quest.progress}/${user.quest.qt}) :clock${Math.floor(user.quest.progress)%12+1}:`;
            ...
            // events
            if (currentEvent) {
                if (currentEvent.type === 'skinfest') {
                    // EVENT: SKINFEST
                    let params = currentEvent.params.split(' ');
                    let category = params[0];
                    let cosmeticID = parseInt(params[1]);
                    let dropChance = parseFloat(params[2]);

                    let r = Math.random() * 100;
                    if (dropChance > r) {
                        // skin drop
                        let categoryCount = await db.cosmetics.getUserCosmeticCategoryCount(msg.author.id, category);
                        if (categoryCount >= 10) {
                            descriptionSuffix += `\n\nAn event skin was dropped, but you don't have room for it :x:`;
                        } else {
                            descriptionSuffix += `\n\nYou got an event skin! Check it out with \`.skins\` :gift:`;
                            db.cosmetics.giveCosmetic(msg.author.id, category, cosmeticID);
                        }
                    }
                }
            }
        ...
        sentEmbed.react('\ud83e\ude9d').then(async() => {
            ...
            collector.on('end', async() => {
                msg.channel.send(embed).then(() => {
                    if (QUEST_COMPLETE) {
                        attemptReply(msg, `**Quest Complete!**\nType \`${PREFIX}quest}\` to claim your rewards!`);
                    }
                });
...
async function sendCollect(msg, args, user) {
    ...
    // Collect Quest
    if (user.quest && user.quest.type === 'collect_coins') {
        user.quest.progress += earnings;
        user.quest.progress = Math.min(Math.round(user.quest.progress*1000)/1000, user.quest.qt);
        await db.users.updateQuest(user.userid, convertQuestToString(user.quest));
        const QUEST_COMPLETE = user.quest.progress === user.quest.qt;
        if (QUEST_COMPLETE) {
            attemptReply(msg, `**Quest Complete!**\nType \`${PREFIX}quest\` to claim your rewards!`);
        }
    }
...
async function sendCollectAll(msg, user) {
    ...
    // Collect Quest
    if (user.quest.type === 'collect_coins') {
        user.quest.progress += totalEarnings;
        user.quest.progress = Math.min(Math.round(user.quest.progress*1000)/1000, user.quest.qt);
        await db.users.updateQuest(user.userid, convertQuestToString(user.quest));
        const QUEST_COMPLETE = user.quest.progress === user.quest.qt;
        if (QUEST_COMPLETE) {
...
async function sendQuest(msg, user) {
    ...
    let questTitle;
    if(user.quest.type === 'catch_tier') {
        questTitle = `Tier Quest - Catch ${user.quest.qt} fish of ${user.quest.tier} tier or higher`;
        questProgress = `${user.quest.progress}/${user.quest.qt}`;
    } else if(user.quest.type === 'catch_weight') {
        questTitle = `Kg Quest - Catch ${kgToKgString(user.quest.qt)} of fish`;
        questProgress = `${user.quest.progress}/${kgToKgString(user.quest.qt)}`
    } else if(user.quest.type == 'catch_fish') {
        questTitle = `Bounty Quest - Catch ${user.quest.qt} ${capitalizeWords(user.quest.name.replace(/_/g, ' '))}`;
        questProgress = `${user.quest.progress}/${user.quest.qt}`;
    } else if (user.quest.type == 'collect_coins') {
        questTitle = `Collect Quest - Collect ${user.quest.qt} coins from your aquariums`;
        questProgress = `${user.quest.progress}/${user.quest.qt}`;
    } else if (user.quest.type === 'use_bait') {
        questTitle = `Bait Quest - Use ${capitalizeWords(user.quest.name.replace(/_/g, ' '))} ${user.quest.qt} times`;
        questProgress = `${user.quest.progress}/${user.quest.qt}`;
    }
    ...
    let options = {
        author: [`${msg.author.tag} (Lvl. ${user.level})`, msg.author.displayAvatarURL()],
        title: questTitle,
        color: api.visuals.getColor('quest', user.quest.type),
        fields: [
        ...
        if(user.quest && user.quest.progress === user.quest.qt) {
            ...
            collector.on('collect', async () => {
                ...
                let options = {
                    ...
                    color: api.visuals.getColor('cmd', user.quest.type),
