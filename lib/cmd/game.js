...
async function sendAquarium(msg, args, user) {
    ...
    let fishTiers = await gameLogic.getAquariumFishTiers(locationID, user);

    // creating embed field
    let fishFound = 0, totalFish = 0;
    let embedFieldVal = '';
    for(const[key, val] of Object.entries(fishTiers)) {
        if(val) {
            fishFound++;
            embedFieldVal += `**(${val.toUpperCase()})** ${capitalizeWords(key)}\n`
        }
        totalFish++;
    }
    // creating embed
