...
async function sendFish(msg, user) {
    ...
    await msg.channel.send(embed).then(async (sentEmbed) => {
        ...
        let selectedFish = possibleFish[0];
        selectedFish.centerCoords = centerCoords;

        let newCanvasBuffer = await createCanvasForFishingPost(locationInfo.id, canvas, selectedFish);
        let options = {
            title: 'REAL FISH',
