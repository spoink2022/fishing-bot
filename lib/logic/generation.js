...
module.exports.generateFish = async function(locationId, BaitData, WeatherData, spawnLegendary) {
    ...
    for (let i=0; i<LocationFishData.length; i++) {
        ...
        if (fish.legendary) {
            if (spawnLegendary) {
                fishOptions.push({
                    id: fish.id,
                    chance: LegendarySpawn[WeatherData.name] * (BaitData && BaitData.families.includes(fish.family) ? 2 : 1) * WeatherData.sizeCurve[fish.sizeClass - 1]
                });
            }
            continue;
        }
    ...
    for (const fishId of generatedFishIds) {
        ...
        let r = rInt/10000 + commonSuffix; // initial r
        r = r * (1 + a) - r**2 * a; // curve for weather ([0 - 1] => [0 - 1])
        ...
        if (rInt % 25 === 0 && BaitData && BaitData.tier !== 'SS') { r += 0.2; } // chance for bonus +0.2 (not with SS bait)
        generatedFish.push({
            id: fishId,
            r: r,
...
module.exports.generateQuest = async function(userLevel, rewardMultiplier) {
    ...
    switch (questType) {
        ...
        case 1: // catch_weight
            ...
            break;
        case 2: // catch_fish
            const locationId = Math.min(Math.max(indexFromLog2Weighting(Math.floor(userLevel/10) + 1), 1), Math.floor(userLevel/10) + 1);
...
module.exports.generateServerShop = function() {
