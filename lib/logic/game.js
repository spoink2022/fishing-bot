...
const { capitalizeWords, percentToRarity, parseQuestString } = require('../misc/str_functions.js');

const TierMinValues = {'D': 0, 'C': 0.3, 'B': 0.6, 'A': 0.75, 'S': 0.9, 'SS': 1};
...
function selectRandom(myList) { // Chances: [(2/3)^n(1/3), ... , (2/3)^2(1/3), (2/3)(1/3), 1/3]
    let r = randint(1, 95);
    for (let i=0; i<myList.length; i++) {
        r -= Math.round((3/4)**i * (1/4) * 100);
        if (r < 0) {
            return myList[myList.length - i - 1];
...
module.exports.getPossibleFishIDs = function(trashChance, fishes, sizeClasses, boostedFamilies, baitUsed, weatherNum) { // generates a list of fish IDs (0 for trash)
    const NUM_OF_FISH = 8;
    let possibleFishIDs = [];
    let validFishes = [];
    let chanceSum = 0;
        ...
        if(!baitUsed && randint(1, 100) < trashChance) { // trash is selected
            ...
            for(let fishChanceObj of validFishes) {
                if(selectionNum <= fishChanceObj.chance) { // get the fish
                    if (baitUsed) {
                        if (!boostedFamilies.includes(fishChanceObj.family)) {
                            reroll = true;
                            break;
                        }
                    }
                    if ((weatherNum === 0 && fishChanceObj.sizeClass >= 3) || ((weatherNum === 4 || weatherNum === 5) && fishChanceObj.sizeClass === 1)) { // weather rerolls (1=sun, 4=rain, 5=storm)
                        reroll = true;
                        break;
                    }
...
module.exports.getPossibleFish = function(ids, minTier, weatherNum) { // takes IDs and returns fish objects
    let possibleFish = [];
    for(let id of ids) {
        if(id === 0) { // add trash to the selection
            ...
            let sizeMult = sizeNum + (Math.random() < 0.03 ? 0.2 : 0.0); // 3% chance of additional 20% multiplier
            // weather influences sizeMult
            const WEATHER_CURVES = [0, 0.4, 0.6, 0.8, 1];
            const CURVE_STRENGTH = 0.3;
            let curveTo = WEATHER_CURVES[weatherNum];
            if (sizeMult < 1 && sizeMult + (curveTo - sizeMult)*CURVE_STRENGTH > minSize) { // only curve non-SS && non-violating bait rules
                sizeMult += (curveTo - sizeMult)*CURVE_STRENGTH;
            }
...
module.exports.getAquariumEarnings = async function(locationID, user, aquariumCapacity) {
    ...
    return Math.min(Math.round(earnings), aquariumCapacity);
}

module.exports.generateQuest = function(userLevel, questMba) {
    ...
    } else if (QuestTypeData.type === 'collect_coins') {
        for(let obj of QuestTypeData.data) {
            if(userLevel < obj.levelUnder) {
                chosenQt = randint(obj.min, obj.max);
                chosenReward = (Math.random()*0.5+0.75) * chosenQt * obj.reward;
    ...
    } else if (QuestTypeData.type === 'use_bait') {
        chosenQt = randint(QuestTypeData.data.min, QuestTypeData.data.max);
        let baitName = selectRandom(api.gamedata.getAllBaitNames());
        let baitInfo = api.gamedata.getBaitData(baitName);
        chosenReward = (Math.random()*0.5+0.75) * chosenQt * baitInfo.value;
        questObj = {
            name: baitName.replace(/ /g, '_')
        };
    }
    ...
    questObj.reward = Math.max(Math.floor(chosenReward*(1 + questMba/100)), 1);
...
module.exports.generateBaitShop = function(startTime) {
    ...
    let BaitNames = api.gamedata.getPurchasableBaitNames();
    console.log(BaitNames);
...
const getStarCount = function(fishCaught) {
...
module.exports.getStarCount = getStarCount;
...
module.exports.getClanPerks = function(clan) {
    // initialize object
    let perks = {
        fish_cd: 0,
        coin_bonus: 0,
    ...
    let stars = getStarCount(clan.fish_caught);
...
module.exports.calculateAvgRingChance = function(ring, ringInfo) {
    let chance = ring.s + ring.m + ring.l + ring.xl;
    for (let i=0; i<4; i++) {
        chance += ringInfo.classBoost[i];
    }
    return Math.round(chance*10/4)/10;
}

module.exports.calculateAvgRingMultiplier = function(ring, ringInfo) {
    ring.consumer = 100 - (ring.premium + ring.sashimi + ring.trophy);
    for (let i=0; i<3; i++) {
        ring[GRADES[i+1]] += ring[GRADES[i]] * ringInfo.gradeBoost[i]/100;
        ring[GRADES[i]] *= 1-(ringInfo.gradeBoost[i]/100);
    }
    let mult = 1 + ring.premium*0.005 + ring.sashimi*0.03 + ring.trophy*0.09;
    return Math.round(mult*10)/10;
}

module.exports.generateRing = function(packType, packInfo) {
    const allRingData = api.fishing.getAllRingData();
    const sellRates = api.fishing.getRingSellRates();
    const packDraws = api.fishing.getRingPackDrawData();
    let ring = {};
    // generate type (+rating)
    let rating;
    let ratingR = randint(1, 100);
    for (let i=0; i<7; i++) {
        if (ratingR <= packDraws[packType][i]) {
            rating = i;
            break;
        }
        ratingR -= packDraws[packType][i];
    }
    let possibleRingTypes = Object.keys(allRingData).filter(key => allRingData[key].rating === rating);
    ring.ring_type = possibleRingTypes[Math.floor(Math.random()*possibleRingTypes.length)];
    // calculate value
    ring.value = Math.round(sellRates[rating]/100 *packInfo.regular.price);
    // generate s, m, l, xl
    for (let abbrev of SIZE_CLASS_ABBREV) {
        let base = packInfo[packType].sizeBase;
        for (let i=0; i<packInfo.flips; i++) {
            if (Math.random() > 0.5) {
                base += 0.5;
            }
        }
    ...
    // generate premium, sashimi, trophy
    for (let grade of GRADES.slice(1)) {
        let base = packInfo.grades[grade][0];
        for (let i=0; i<packInfo.grades[grade][1]; i++) {
            if (Math.random() > 0.5) {
                base += 1;
            }
        }
